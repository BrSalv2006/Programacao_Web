...
async function apiFetch(url, options = {}) {
	...
	try {
		...
		const isLotesEndpoint = urlObj.pathname === '/api/lotes'
		...
		if (response.ok && !isWriteOperation && options.method !== 'POST') {
			...
			cloned.json().then(async data => {
				await saveOfflineData('api_cache', { url: cacheKey, data: data })
				if (isLotesEndpoint && !viewParam && data.data && Array.isArray(data.data)) {
		...
		if (isWriteOperation) {
			...
			const cached = await getOfflineDataByKey('api_cache', cacheKey)
			if (cached) {
